...
export async function POST(req: NextRequest) {
  ...
  // Load work diaries for this order
  ...
  const diaries: DiaryForProfitability[] = (rawDiaries ?? []).map((row) => {
    ...
    const hasLeader = typeof d.crewLeaderName === "string" && d.crewLeaderName.trim() ? 1 : 0;
    return {
      id: row.id as string,
  ...
  // Load inventory consumptions + catalog cost prices
  ...
  // Upsert to profitability_snapshots

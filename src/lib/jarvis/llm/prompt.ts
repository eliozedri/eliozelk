...
export function buildIntentPrompt(req: LLMRequest): { system: string; user: string } {
  ...
  const system = [
    ...
    "- tool_connection_request: connect/integrate a new tool/provider/capability ('תחבר לי כלי תמונות', 'תחבר את נאנו בננה', 'תוסיף יכולת X', 'connect/integrate service').",
    ...
    "- image_creation: GENERATE a new image ('תיצור לי תמונה', 'תעשה תמונה', 'תייצר תמונה', 'נאנו בננה', 'בסגנון הזה'). image_editing: EDIT an image ('תערוך את התמונה').",
    "- operations_risk_report: 'what could block jobs this week', a broad multi-area risk request.",
    "",
    "MEDIA IS CONTEXT, NOT INTENT: an attached image does NOT mean OCR. Decide from the caption — 'create/edit an image' (even with an attached image) → image_creation/image_editing; only 'read/scan/extract text' → ocr_document. If a tool like 'Nano Banana' is named, it is image_creation, never a document scan.",
...
const VALID_INTENTS = new Set<string>([
  ...
  "development_request", "tool_connection_request", "general_assistant", "image_creation", "image_editing",

...
export function computeDiagnostics(
  ...
  if (missingBillingPct > 0.15 && dataQuality.totalDiaries >= 3) {
    ...
    findings.push({
      ...
      severity,
      department: "office",
      title: `${dataQuality.missingBilling} יומנים ללא סכום חיוב (${pct(missingBillingPct * 100)})`,
      ...
      recommendation: "הדרך את הצוות למלא סכום חיוב בכל יומן לפני הגשה.",
  ...
  if (dataQuality.completenessScore < 60 && dataQuality.totalDiaries >= 3) {
    ...
    findings.push({
      ...
      explanation: `חסרים: ${dataQuality.missingBilling} ללא חיוב · ${dataQuality.missingCrew} ללא פרטי צוות · ${dataQuality.missingTime} ללא שעות · ${dataQuality.missingOrderLink} לא מקושרים להזמנה.`,
      recommendation: "קבע תקן מינימלי למילוי יומנים לפני הגשה: חיוב, שעות, צוות.",

...
import {
  ...
  LogOut, X, Cable, ScanLine, ScanText, Send, Store, Truck, Wallet, Bell,
} from "lucide-react";
...
const NAV_SECTIONS: NavSection[] = [
  ...
  },
  {
    ...
    items: [
      { tabId: "catalog", href: "/catalog", label: "קטלוג מוצרים", icon: <Database className={ICON_CLS} />, matchFn: (p) => p.startsWith("/catalog") },
...
export function Sidebar({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <aside className="ek-sidebar w-64 lg:w-56 h-full flex flex-col shrink-0">
      ...
      {/* Navigation */}
      <nav className="flex flex-col p-2.5 flex-1 min-h-0 overflow-y-auto overscroll-contain [-webkit-overflow-scrolling:touch]">
